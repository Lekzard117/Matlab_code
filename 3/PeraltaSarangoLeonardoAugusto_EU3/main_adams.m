...
Y_rk = rKutta_SistemaEc( a, a + 2*h, y0, 3, h); % Runge-Kutta para y0, y1, y2
%[x_e, Y_e] = eulerSistemaEc(f, a, a + 2*h, y0, 2);     % Euler para y0, y1, y2
Y_h = heun_SistemaEc(a, a + 2*h, y0, 2, h);
...
y2_val = Y_rk(3);       % y2 con Euler
y3_val = Y_rk(4);

% Valores iniciales para Adams-Bashforth
x_inicial = [a, a + h, a + 2*h, a+2*h];  % Puntos x0, x1, x2
y_inicial = [y0_val, y1_val, y2_val, y3_val];  % Valores y0, y1, y2, y3
